...
function product_search($keyword,$slide=1){
    $start = ($slide-1)*4;
    return pdo_query("SELECT * FROM sanpham WHERE TenSP LIKE '%$keyword%' LIMIT $start,4");
}
function product_searchTotal($keyword){
    return pdo_query_value("SELECT COUNT(*) FROM sanpham WHERE TenSP LIKE '%$keyword%'");
}
function partner(){
    return pdo_query("SELECT * FROM doitac");
}
// admin
function getAll_productService(){
    return pdo_query("SELECT * FROM sanpham; UNION SELECT * FROM dichvu");
}
